# Agent Guide — ep_disable_error_messages
...
`ep_disable_error_messages` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_disable_error_messages`:
